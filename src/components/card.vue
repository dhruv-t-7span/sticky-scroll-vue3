<template>
  <section class="absolute inset-0">
    <div
      class="flex flex-col lg:h-full md:flex-row space-y-4 lg:space-y-0 md:gap-6 lg:gap-12 xl:gap-24"
    >
      <div
        class="flex-1 flex items-center transition-opacity duration-300 order-1 lg:order-none"
      >
        <div class="space-y-3">
          <div class="relative inline-flex text-indigo-500 font-semibold">
            {{ id }} / {{ totalLength }}
            <svg
              class="fill-indigo-300 absolute top-full w-full"
          ...
          <h2 class="text-4xl text-slate-900 font-extrabold">
            {{ title }}
          </h2>
      ...
      <div class="flex-1 flex items-center transition duration-300">
        <img
          width="512"
          height="480"
          :src="imageUrl"
